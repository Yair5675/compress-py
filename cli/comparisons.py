...
compressors_to_test: tuple[tuple[str, Compressor], ...] = (
    ...
    ("LZW (best compression rate)", LzwCompressor(int(cli.lzw.DictionarySize.EXTRA_LARGE), OutOfMemoryStrategy.USE_MINIMUM_REQUIRED)),
...
def test_with(input_path: Path, compressor: Compressor) -> BenchmarkResults:
    benchmark = CompressorBenchmark(compressor)
...
def get_info_table(results: tuple[BenchmarkResults]) -> Table:
    info_table: Table = Table(
        title="Combined Benchmark Results", style="bold blue", title_style="bold white", box=box.ROUNDED, show_lines=True
    )
...
def compare_all(input_path: Annotated[Path, typer.Argument(
    ...
    results: tuple[BenchmarkResults] = tuple(map(
        lambda compressor_to_test: test_with(input_path, compressor_to_test[1]), compressors_to_test)
    )
