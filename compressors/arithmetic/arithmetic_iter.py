...
class StateCallback(ABC):
    ...
    def process_interval(self, interval: interval_state.Interval) -> bool:
        ...
        After the method is executed, it returns a boolean. If true - the method's caller should call the method again
        (it indicates the interval was mutated). If false - do not call it again right after the method is finished.
        ...
        :return: True if the interval was mutated and the method should be called on it (and its new state) again, False
                 if it should not be called again until something else updates it.
        """
        pass
...
class ArithmeticIterator:
    ...
    def process_prob_interval(self, probability_interval: ProbabilityInterval) -> None:
        ...
        use_callback = True
        while use_callback:
            use_callback = self.state_callback.process_interval(self.current_interval)
